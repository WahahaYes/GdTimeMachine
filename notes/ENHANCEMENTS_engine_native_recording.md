...
Status: Op 1-3 shipped (2026-08-01) — 43 green Op 1, 66 green Op 2, Op 3 measured at 16-18 fps foreground / 1 fps backgrounded (see `SPIKE_screenshot_fps.md`). Scene-aware profiles + config-store refactor shipped same day (see `SESSION_scene_aware_profiles.md`) — that session also delivered the format dropdown + 4 GB dock warning (#5) and the `EditorDock` migration (see `SESSION_editordock_migration.md`). 106/106 GUT green. Op 4 shipped — #5 dropdown + #6 `project.godot` pollution fixed via restore-on-stop (metadata approach NO-GO, see `SPIKE_movie_metadata.md`). **Op 5 shipped 2026-08-02 (152/152)** — `BackendScreenshotCapture` core + IN_PLACE handling. **Op 6 shipped 2026-08-03 (199/199)** — #4a ffmpeg tier-2 hook (`ffmpeg_convert.gd`) + MP4/WebM tier-2 targets in the dropdown; verified end-to-end. **Op 7 shipped 2026-08-07 (215/215)** — editor-wide record/stop shortcut (`EditorSettings.add_shortcut` + `BaseButton.shortcut`; API correction: the command palette binds no keys), live recording state (backend, elapsed, output path) in the dock status line (reframe: no `get_editor_status_bar()` in 4.7), per-backend capture-semantics tooltips (fixed-fps vs real-time). **Post-Op7 hardening 2026-08-07** — hard AVI 4 GB size guard in the Movie Maker backend (see §5), record/stop shortcut fallback to the dock's button when the run bar can't be traversed, null-dock guard in the plugin toggle, dead `recording_progress` signal removed. (Note: the `make test-godot` fail-loud wrapper and `.github/workflows/test.yml` CI were tried during hardening and deliberately dropped — verification stays local via `godot --headless -s --path . addons/gut/gut_cmdln.gd -gexit`.) Remaining: OBS (Phase 3/4), CLI companion.
...
- AVI (MJPEG + uncompressed audio): current default; **capped at 4 GB** — a long/high-res recording can hit the cap and break the file. Guard shipped in two layers: a dock warning ("AVI is capped at 4 GB…") *and* (2026-08-07) a hard size guard in `BackendMovieMaker` — the backend polls the output file size every 0.5 s while recording and gracefully stops 256 MiB before the cap (4 GiB − 256 MiB), emitting a "Stopped at the AVI 4 GB cap" notice so the file finalizes inside the limit instead of corrupting. Applies to the tier-2 AVI intermediate too (MP4/WebM targets).
...
## 7. `[x]` Nice-to-haves — SHIPPED 2026-08-07

- Editor-wide record/stop shortcut — no need to hit the tiny run-bar button. **API correction vs earlier research:** the command palette was the wrong mechanism — `EditorCommandPalette.add_command`'s 4th param is display-only and binds no keys. The maintained editor-wide shortcut mechanism (4.6+, PR #102889) is `EditorSettings.add_shortcut(path, shortcut)` + `BaseButton.shortcut` on the run-bar button: user-rebindable under Project > Editor Settings > Shortcuts, survives plugin disable/re-enable, fires editor-wide via the button's `shortcut_input()`. Default Ctrl+Alt+R (Cmd+Alt+R on macOS — Meta instead of Ctrl); deliberately uncommon so it doesn't shadow run-bar (F5–F8) or script-editor chords. Command-palette action "GdTimeMachine: Toggle Recording" added for discoverability (display-only text) and removed on `_exit_tree`. Pure `build_toggle_shortcut(use_meta)` static → both platform branches GUT-tested.
- Live recording state in the editor status line while recording. **Reframed:** `EditorInterface.get_editor_status_bar()` **does not exist** in Godot 4.7 (no such method in the class docs; `EditorStatusBar` XML 404 at 4.7-stable and master) — the thin bottom strip is internal layout and must not be traversed. The maintained bottom UI is `EditorDock` bottom-slot, which the addon already uses, so the live state lives in the dock's existing status row: `Recording [backend] → file (mm:ss)`, refreshed by a 1s lazy timer, stopped on stop/error/converted so the Converting…/Saved flows are unchanged. `_now()`/`_fake_time` seam keeps the clock GUT-fakeable.
- Dock backend tooltip explains capture semantics per backend (constraint 9): `BackendMovieMaker.get_description()` now states **fixed-fps, non-real-time** — the scene is restarted and playback is deterministically paced, so output rate equals the configured FPS regardless of editor performance; `BackendScreenshotCapture.get_description()` states **real-time** — the sim runs at normal speed, capture is machine-bound (~15 fps typical), no audio, window must stay visible/focused. Contract locked by GUT (description must mention the semantics keyword).
- Tests: `test_plugin_shortcut.gd` (6), dock live-status (8), backend description contract (2) — 215/215 GUT green (was 199). Manual windowed check in `SESSION_PLAN_engine_native_recording.md` Open items.
