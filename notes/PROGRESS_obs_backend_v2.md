...
- The `backend_obs.gd` narration docs referenced a 282-test suite that the rebuilt history didn't match (280 tests): the two launch/ownership narration tests were the gap. Since fixed under this session's reconciliation — `test_backend_obs.gd` is back to 28 tests (`test_ensure_obs_running_notices_launch_progress`, `test_ensure_obs_running_does_not_notice_when_no_launch_needed` restored per the launch narration spec), and the narration + auto-close code are committed in HEAD. Re-run the full suite before merge to confirm the 282 total.

## OBS lifecycle hardening 2026-08-16 — orphaned OBS can never outlive its godot parent (D1–D4)

### Motivation (two real leaks, one day)

An OBS we launched at 10:22 survived ~7 h after its godot parent died — the only cleanup path was `_exit_tree` (skipped on hard godot death; nothing reaped it on the next startup), and `OS.kill(pid)` = SIGTERM one-shot never forced a second OBS (PID 77353) dead — `kill -9` was needed. The fast path silently reused the leftover, so the "missing terminal log" was really "OBS was already running". Full forensics in `notes/DESIGN_obs_lifecycle_hardening.md` (D1–D4 below).

### What landed (all in `backend_obs.gd` unless noted)

- **D1 — launch ledger**: `user://gdtime_obs_launched.cfg` (ConfigFile, `obs/pid` + `obs/binary`) written on every successful launch via seam `_persist_launched_pid`, read via `_load_launched_ledger`, dropped via `_clear_launched_ledger`. Cleared on timeout only when kill is confirmed; deliberately left in place on graceful close so the next startup's sweep proves death.
- **D2 — startup orphan sweep**: `_reap_orphaned_launches()` runs fire-and-forget from `_ready()` *before* the editor gate (headless drive tool gets it too — `tools/obs_backend_drive.gd` DriverBackend now calls it explicitly). Gates on `auto_close`; no ledger entry → no-op; dead pid → clear silently; alive but not provably ours → `push_warning` + clear, never kill; alive + ours → TERM → 2 s grace → SIGKILL → `[GdTM]` print (or warning if it survived) + clear.
- **D3 — kill escalation**: `_kill_process_and_wait(pid)` = TERM via existing `_kill_process`, then up to 2 s polling `_is_process_alive` (= `OS.is_process_running`) via the `_sleep` seam, then `_force_kill_process` (`kill -9`/`taskkill /F`). Timeout path in `ensure_obs_running()` uses it and clears the ledger only when confirmed dead. `_exit_tree` keeps best-effort TERM (recording already stopped) and now *leaves* the ledger for the sweep.
- **D4 — reuse notice (approved earlier in-session)**: fast path emits `"OBS Studio is already running — reusing it."` so a leftover instance is visible in the terminal instead of manifesting as silent reuse.
- **Ownership proof**: `_process_is_ours(pid, binary)` reads `/proc/<pid>/cmdline`, requires our exact binary as argv[0] + `--minimize-to-tray` (pid-recycle kills impossible on Linux); non-Linux best-effort documented. Safety note: only the pid *we* recorded is ever touched — a user's manual OBS (e.g. the external `sh -c` spawn from 17:14) is never a target.
- **Parse-warning fix (validation only)**: the first implementation used `cmdline.split(char(0))`; Godot 4.7 emits "Unicode parsing error … Unexpected NUL character" warnings at compile time for any NUL-bearing string construct (`char(0)`, `\u0000` literals). Switched to `cmdline.split(String.chr(0))` — identical runtime semantics, zero warnings.

### Tests (`test_backend_obs.gd`, 28 → 35)

`ReapTestBackend` + `DiesAfterTermBackend` fakes (in-memory ledger, fake clock, settable alive/ours — no real `user://` or OS access; `SettingsBackend` replaces the one raw `BackendOBS.new()` so no test run can touch the real ledger; Recording/Launch backends got in-memory seam overrides). Replaced the fast-path-silence test with `test_ensure_obs_running_notices_reuse_when_already_reachable`. New: sweep reaps live-ours (TERM→SIGKILL), dies-after-TERM (no force), leaves not-ours alone, clears dead ledger, no-op on empty ledger, no-op when auto_close off, `_kill_process_and_wait` escalates. Timeout test extended with ledger-cleared asserts; launch test asserts ledger persisted.

### Validation

- Isolated `test_backend_obs.gd`: **35/35 ×2**.
- Full suite ×2: **289/289 passing, exit 0**, and **zero** "Unicode parsing error" warnings in the run.
