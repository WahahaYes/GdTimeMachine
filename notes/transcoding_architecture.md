...
`plugin.gd` migrates `ffmpeg/* → transcoders/ffmpeg/*` once (copy-if-absent, keep old keys read as fallback for one version, then drop). Recorder backends read `auto_convert`/`clean` from the *active* transcoder's section via the registry, not hardcoded `ffmpeg/` paths.
