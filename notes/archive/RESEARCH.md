...
1. [Cross-Platform Feasibility](#2-cross-platform-feasibility)
1. [Existing Tooling Landscape](#3-existing-tooling-landscape)
1. [Editor Integration Approaches](#4-editor-integration-approaches)
1. [Architecture Brainstorming](#5-architecture-brainstorming)
1. [Open Questions & Next Steps](#6-open-questions--next-steps)
...
| File | Purpose | |------|---------| | `capture_obs.py` | CLI entrypoint: launches Godot scene, connects OBS, records, cleans up | | `obs_controller.py` | OBS WebSocket controller: creates PipeWire capture source, manages recording | | `capture_all_showcase.sh` | Orchestrator: iterates over manifest of commits, uses git worktrees, builds Rust, runs per-commit captures |
...
| Platform | OBS Studio | WebSocket built-in? | Notes | |----------|-----------|---------------------|-------| | **Linux** | ✅ | ✅ (28+) | Native package or Flatpak. PipeWire for Wayland, Xcomposite for X11. | | **Windows** | ✅ | ✅ (28+) | DirectX capture. Most mature platform for OBS. | | **macOS** | ✅ | ✅ (28+) | Screen Capture API. Deprecated old method, newer macOS (13+) uses different capture path. |
...
| Capture Method | Linux | Windows | macOS | |---------------|-------|---------|-------| | PipeWire (Wayland) | ✅ | ❌ | ❌ | | Xcomposite (X11) | ✅ | ❌ | ❌ | | DirectX / BitBlt / WGC | ❌ | ✅ | ❌ | | macOS Screen Capture | ❌ | ❌ | ✅ | | Display Capture (AVFoundation) | ❌ | ❌ | ✅ |
...
1. Have OBS Studio installed with WebSocket enabled

### Cross-platform summary

| Concern | Linux | Windows | macOS | |---------|-------|---------|-------| | OBS install | Easy (package mgr) | Easy (installer) | Easy (DMG) | | WebSocket | Built-in 28+ | Built-in 28+ | Built-in 28+ | | Python | Usually pre-installed | Manual install | Usually pre-installed | | Screen capture | PipeWire / Xcomposite | DirectX / BitBlt / WGC | macOS Screen Capture | | Window matching | Limited on Wayland | Full (exe name, title) | Partial | | **Verdict** | ✅ Good (what we have) | ⚠️ Needs different impl | ⚠️ Needs different impl |
...
| Need | Existing solutions | Gap | |------|------------------|-----| | OBS control from GDScript | ✅ `obs-websocket-gd` | Works at runtime, not editor | | Editor plugin for recording | ⚠️ Godot OBS Recorder (3.5 only) | No maintained Godot 4 solution | | Automated multi-commit capture | ✅ Our scripts/ | Linux-only, Python-dependent | | One-click "record my game" from editor | ❌ Nothing maintained | Clear gap | | Cross-platform screen capture setup | ❌ Not in any existing tool | Hard problem |
...
- Pro: can match by executable name — "godot" / "godot\_\*" patterns work well
...
1. **OBS WebSocket protocol complexity**: The WebSocket auth handshake (identify with HMAC-SHA256) is non-trivial in GDScript. Does `obs-websocket-gd` handle this correctly for v5? (Yes, it targets obs-websocket 5.x.)

1. **Wayland token capture UX**: On Wayland, the first capture setup requires an interactive xdg-desktop-portal dialog (user selects monitor). How do we handle this from within Godot? Portal doesn't work headless. Could:
...
1. **Godot 4 built-in Movie Maker**: Godot has a "Movie Maker" mode that writes video directly. Why not use it instead?
...
1. **Editor plugin vs. runtime addon**: Our use case is primarily **editor-based** (recording footage during development). But the addon could also work at runtime (e.g., player recording/streaming from within the game). Should we target both?
...
1. **Medium-term** (possible public addon):
