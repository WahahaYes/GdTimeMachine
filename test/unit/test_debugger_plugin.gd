...
class PluginBehaviorMirror:
	...
	# Verbatim port of debugger_plugin.gd:_capture so the mirror rejects and
	# accepts exactly what the real plugin does (see the parity test): messages
	# may arrive as full "game_view:get_screenshot" or stripped
	# "get_screenshot", ids/dims may be int/float/numeric-string, and paths may
	# be .png/.jpg/.jpeg.
...
func test_screenshot_capture_guard_present_in_source() -> void:
	...
	# when idle. Two independent narrow pins (reformat-safe) prove both
	# symbols gate the claim.
	var source := FileAccess.get_file_as_string(PLUGIN_SOURCE_PATH)
	assert_true(source.contains("SCREENSHOT_CAPTURE_PREFIX"))
...
func test_capture_accepts_game_view_prefixed_and_jpg_paths_parity() -> void:
	# Drives the ported mirror through each variant the real plugin accepts.
	# All variants must emit screenshot_received with correctly parsed fields.
	var plugin := _make_mirror()
	plugin.set_screenshot_capture_active(true)
	plugin._last_screenshot_rq_id = 7
	var received: Array = []
	plugin.screenshot_received.connect(func(id, w, h, p): received.append([id, w, h, p]))

	# 1. Full "game_view:get_screenshot" prefix, int id/dims, .png path
	received.clear()
	assert_true(plugin._capture("game_view:get_screenshot", [1, 1280, 720, "user://frame.png"], 0))
	assert_eq(received, [[1, 1280, 720, "user://frame.png"]])

	# 2. Stripped "get_screenshot", float id/dims, .jpg path
	received.clear()
	assert_true(plugin._capture("get_screenshot", [2.0, 1920.0, 1080.0, "user://cap.jpg"], 0))
	assert_eq(received, [[2, 1920, 1080, "user://cap.jpg"]])

	# 3. Full prefix, numeric-string id/dims, .jpeg path
	received.clear()
	assert_true(
		plugin._capture("game_view:get_screenshot", ["3", "640", "480", "user://img.jpeg"], 0)
	)
