...
func _setup_run_bar_button() -> void:
	...
	if row == null:
		push_warning("GdTimeMachine: editor run bar not found; run-bar record button unavailable (bottom-panel tab still works)")
		return
...
func _setup_game_view_button() -> void:
	...
	if row == null:
		push_warning("GdTimeMachine: game view toolbar not found; toolbar record button unavailable (bottom-panel tab still works)")
		return
	...
	if existing is Button:
		...
		_apply_button_state(_game_view_button, _recorder_controller.is_recording())
		return
	...
	_apply_button_state(_game_view_button, false)
...
func _build_record_button() -> Button:
	...
	button.flat = true
	button.tooltip_text = "GdTimeMachine — Start/stop recording (uses the dock settings)"
